...
- [ ] לעצב את המעטפת המשותפת: כותרת יחידה, ניווט תחתון ופעולת הוספה, תוך שימור מנגנון ה־safe-area הקיים.
- [ ] לרכז חלבון וקצב בכרטיס ראשי קומפקטי; להציג קלוריות ומים בכרטיסים משניים ולתת עדיפות ליומן הארוחות בהמשך.
- [ ] לשמר בחירת יום, ארבע קבוצות ארוחה, פירוט פריט, מצבי יום ריק ויעד שהושג. אין להציג קצב של היום בתאריך היסטורי.
- [ ] לשמר הוספת מים 250/500/700 מ״ל, כמות מותאמת, ביטול ושגיאות; אפשר לרכז אפשרויות תחת ״אחר״ אך לא להסירן.
- [ ] אם ההמלצה מקבלת פעולה, היא תפתח את זרימת ההוספה המהירה הקיימת; לא תבצע שמירה אוטומטית של כמה מאכלים.
...
- [ ] לאחד כותרת, בורר שבוע/חודש, ממוצע וגרף לאזור ראשי אחד; להעביר רצף וממוצע קלוריות לכרטיסים משניים.
- [ ] לשמר את סדר התאריכים, קו היעד והנתונים הקיימים. גרף חודשי אינו הופך אוטומטית את הממוצעים השבועיים לממוצעים חודשיים: הכותרות יציינו זאת במפורש.
- [ ] לא להעתיק את תובנת ״עלייה של 8%״ מההמחשה — אין לה חישוב קיים. להסיר גרף קישוט שאינו מבוסס נתונים במקום להציגו כמדידה.
- [ ] לתת לגרף תיאור נגיש וערכים קריאים, בלי להסתמך רק על גובה עמודה או צבע.
...
- [ ] לעצב סיכום של ארבעת ערכי היעד וקבוצות ארוחה שניתן לפתוח ולסגור באמצעות controls נגישים.
- [ ] לשמר את מלוא הנוסח הקיים, הכמויות, המידות הביתיות וחמש הקבוצות, כולל אבקת החלבון. הקיצור בתמונה אינו אישור למחיקת תוכן.
- [ ] להשאיר את התפריט הנחיה סטטית; לא לחבר את יעדיו לפרופיל או לרשומות היומיות.
- [ ] להסיר פעולת הוספה צפה מהמסך; לשמר ניווט ושמירה על מיקום גלילה בזמן פתיחת קבוצה.
...
- [ ] לעצב חיפוש, מספר תוצאות ופעולת ״מאכל חדש״ מעל רשימה מקובצת חד־עמודתית.
- [ ] להפוך את שורת המאכל לפעולת עריכה נגישה, עם שם, מידה וערכי חלבון/קלוריות קריאים.
- [ ] להשתמש בחיפוש ובנרמול הקיימים; להבדיל בין קטלוג ריק לבין חיפוש ללא תוצאות.
- [ ] לשמר פתיחת העורך הקיים בלבד: בחירת מאכל כאן אינה רישום אכילה ואינה משנה את מודל הקטלוג המשותף.
...
- [ ] לעצב את מעטפת ההוספה המשותפת לשלושת המצבים: כותרת, סגירה, טאבים ובחירת יום/סוג ארוחה.
- [ ] לעצב חיפוש ורשימת מאכלים בעמודה אחת, ולאחר בחירה להציג כמות וסיכום לפני אישור.
- [ ] לשמר חישובי מנות/גרמים, יום היעד, סוג הארוחה והתנהגות השמירה הקיימים.
- [ ] לטפל בפוקוס בתוך החלונית ובהחזרתו לסגירה, במקלדת ובגלילה. לא להציג ידית גרירה אם אין מחוות גרירה אמיתית.
...
- [ ] לקבץ שם, חלבון וקלוריות; להפריד בין ערכים למנה לבין כמות ומידה, ולהציג את הסיכום המחושב הקיים.
- [ ] לשמר מידה מותאמת, משקל אופציונלי ואת תנאי החובה הקיימים; להציג שגיאות סמוך לשדה המתאים.
- [ ] לעצב את השמירה לקטלוג כבחירה מפורשת. ברירת המחדל נשארת כבויה, גם אם המתג בהמחשה דלוק.
- [ ] להצמיד את פעולת השמירה לתחתית החלונית בלי להסתיר שדות, הודעות או תוכן בעת פתיחת מקלדת.
...
- [ ] לעצב בחירה ממצלמה או מגלריה, תצוגה מקדימה, מצב ניתוח ותוצאה ניתנת לעריכה.
- [ ] להציג confidence והערה כפי שחוזרים כיום, בלי לייצר תחושת דיוק ודאית. לשמר החלפת תמונה, הנחיות וטיוטה לאורך הזרימה.
- [ ] להשתמש בטופס הקיים לשם ולערכי התזונה. ״מנה אחת״ בהמחשה אינה סיבה להוסיף חישוב כמויות חדש לתוצאת AI.
- [ ] לשמר את מצבי מכסה יומית, שגיאת ניתוח/רשת/אימות והמעבר להזנה ידנית; שמירה לקטלוג נשארת opt-in.
