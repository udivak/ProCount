...
כל שלב מתאים להמחשה אחת, לפי סדר התמונות `00`–`10`. היישום המקומי הושלם לפי הסעיפים המסומנים; אין בכך אישור לפריסה או שינוי מודל הנתונים.
...
- [x] ליישר את העורך עם שפת טופס ההוספה: שם, מידה, חלבון וקלוריות בקבוצות ברורות ושמירה נגישה.
- [x] לשמר את ההבחנה בין יצירה לעריכה; אפשרות הערכת AI נשארת רק ביצירת מאכל חדש.
- [x] לשמר טיוטה, נעילות, הודעות על שימוש ברשומה קיימת והתנהגות ניסיון חוזר לאחר שמירה חלקית.
- [x] לעצב פעולה הרסנית בנפרד ואת דיאלוג האישור הקיים; להוסיף טיפול בפוקוס בלי לעקוף אישור, נעילה או הודעת שגיאה.
...
- [x] לעצב כותרת מאכל ונתוני תזונה לקריאה בלבד, עם קבוצות לכמות, חלבון ל־100 גרם, סך חלבון, קלוריות ומקור.
- [x] לשמר `—` כשמשקל או ערך נגזר אינם זמינים; לא להסיק משקל שלא קיים ברשומה.
- [x] להציג שינוי סוג ארוחה כפעולה נפרדת עם הבחירה הנוכחית, ולשמר מצבי שמירה ושגיאה.
- [x] להפריד מחיקה מהתוכן ולהעבירה דרך דיאלוג האישור; להחיל את כללי הפוקוס והסגירה של החלוניות.
...
- [x] לקבץ יעדי חלבון ומים עם steppers, שם וחשבון תחת פרופיל, והתנתקות באזור נפרד.
- [x] לשמר טווחי יעד וצעדים קיימים, שמירת שם והתנהגות שגיאות; אין שינוי בפרופיל או בשכבת הנתונים.
- [x] להסיר ניווט תחתון ו־FAB מהגדרות, וליישר חזרה/סגירה עם RTL ועם יתר האפליקציה — לא להעתיק כיוון חץ שגוי מתמונה.
- [x] לבצע מעבר מלא בכל 11 המסכים: התאמה חזותית, RTL, ניווט במקלדת, טקסט מוגדל, reduced motion/transparency/contrast, מצבי ריק ושגיאה.
- [ ] לאמת VoiceOver וב־iPhone פיזי כ־PWA מותקן: cold launch, portrait/landscape, מקלדת, פתיחה וסגירה של חלוניות ו־relaunch.
